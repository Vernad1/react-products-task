import { FC } from "react";
import { IProduct } from "../../models/IProduct";
import styles from "./productList.module.css";
import { Link } from "react-router-dom";
import {
  removeProduct,
  toggleProductLike,
} from "../../store/reducers/product/ProductSlice";
import { useAppDispatch } from "../../hooks/redux";

interface Props {
  products: IProduct[];
}

export const ProductsList: FC<Props> = ({ products }) => {
  const dispatch = useAppDispatch();

  function handleButtonLikeClick(
    e: React.MouseEvent<HTMLButtonElement, MouseEvent>,
    id: number
  ) {
    e.preventDefault();
    dispatch(toggleProductLike(id));
  }

  function handleButtonRemoveClick(
    e: React.MouseEvent<HTMLButtonElement, MouseEvent>,
    id: number
  ) {
    e.preventDefault();
    dispatch(removeProduct(id));
  }

  return (
    <ul className={styles.productsList}>
      {products.map((product: IProduct) => {
        return (
          <li className={styles.productsItem} key={product.id}>
            <Link
              to={`/react-products-task/products/${product.id}`}
              className={styles.link}
            >
              <div className={styles.imageWrapper}>
                <img
                  src={product.images[0]}
                  width={300}
                  height={300}
                  loading="lazy"
                  alt="character"
                  className={styles.productsImage}
                ></img>
              </div>

              <div className={styles.productsInfo}>
                <div className={styles.productsName}>{product.title}</div>
                <div className={styles.productsStatus}>
                  Цена: {product.price}$
                </div>
                <button
                  onClick={(e) => handleButtonLikeClick(e, product.id)}
                  className={styles.productsLike}
                >
                  <svg
                    viewBox="0 0 24 24"
                    fill={product.isLike ? "red" : "none"}
                    width={24}
                    height={24}
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      fillRule="evenodd"
                      clipRule="evenodd"
                      d="M12 6.00019C10.2006 3.90317 7.19377 3.2551 4.93923 5.17534C2.68468 7.09558 2.36727 10.3061 4.13778 12.5772C5.60984 14.4654 10.0648 18.4479 11.5249 19.7369C11.6882 19.8811 11.7699 19.9532 11.8652 19.9815C11.9483 20.0062 12.0393 20.0062 12.1225 19.9815C12.2178 19.9532 12.2994 19.8811 12.4628 19.7369C13.9229 18.4479 18.3778 14.4654 19.8499 12.5772C21.6204 10.3061 21.3417 7.07538 19.0484 5.17534C16.7551 3.2753 13.7994 3.90317 12 6.00019Z"
                      stroke={product.isLike ? "red" : "#000000"}
                      strokeWidth="2"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                    />
                  </svg>
                </button>
                <button
                  onClick={(e) => handleButtonRemoveClick(e, product.id)}
                  className={styles.productsDelete}
                ></button>
              </div>
            </Link>
          </li>
        );
      })}
    </ul>
  );
};
